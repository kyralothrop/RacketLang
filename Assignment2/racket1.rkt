...
(define (eval-1 exp)
  (cond ((constant? exp) exp)
    ...
    ; Exercise 5_________________
    ; if there are too many arguments, show error
    ((quote-exp? exp) (if (= (length exp) 2) (cadr exp) (error "Too many arguments: " exp)))
    ; Exercise 6_________________
    ; apply map to the given arguments
    ((map-exp? exp)
     (if (= (length exp) 3) (map (eval(cadr exp)) (eval(caddr exp)))
         (map (eval(cadr exp)) (eval(caddr exp)) (eval(cadddr exp)))
         ))
    ; Exercise 7__________
    ; handle all cases given to and, length 1, length 2(one expression), and multiple expressions
    ((and-exp? exp)
     (cond
       [(= (length exp) 1) #t]
       [(= (length exp) 2) (eval-1 (cadr exp))]
       [else (if(eval-1 (cadr exp)) (eval-1 (caddr exp)) #f)]
       )
     )
...
; Exercise 6 helper procedure_________
(define map-exp? (exp-checker 'map-1))
; Exercise 7 helper procedure_________
